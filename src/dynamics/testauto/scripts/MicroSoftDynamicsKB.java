...
public class MicroSoftDynamicsKB {
	...
	public void testMicroSoftDynamicsRecTS() throws Exception {
		...
		if (rcrscUI.isDisplayed()) {
			...
			Thread.sleep(3000);
			if (driver.findElement(By.xpath("//input[contains(@class, 'title-input form-control input-sm')]"))
					.isDisplayed()) {
				System.out.println("Found the title input holder..");
				driver.findElement(By.xpath("//input[contains(@class, 'title-input form-control input-sm')]"))
						.sendKeys("Replay-Dynamics UI SEP22");
				Thread.sleep(3000);
			}
