...
public class RecordZendeskTicketTC {
	...
	public void setUp() throws Exception {
		...
		/*
		 * How to Installing extensions via ChromeDriver
		...
		JOptionPane.showMessageDialog(null, "Please click on the Cancel button and manually select a Zendesk UI.");
	...
	public void testValidateZendeskRecord() throws Exception {
		...
		/*
		 * if (temp.isDisplayed()) {
		 * System.out.println("Zendesk button appeared!"); WebElement
		 * recurButtton = driver.findElement(By.
		 * xpath("//div[contains(@class, 'branding-logo')]"));
		 * 
		 * Actions action3 = new Actions(driver);
		 * 
		 * action3.moveToElement(recurButtton).build().perform(); Robot uiBtn =
		 * new Robot(); uiBtn.mouseMove(95, 1640);
		 * 
		 * Thread.sleep(3000); action3.click(recurButtton).build().perform();
		 * 
		 * System.out.println("Zendesk button clicked!"); // WebElement startRec
		 * = driver.findElement(By.linkText("Start // Recording")); //
		 * startRec.click(); /* Actions action4 = new Actions(driver);
		 * 
		 * action4.moveToElement(startRec).build().perform();
		 * 
		 * Thread.sleep(3000); Robot uiBtn2 = new Robot(); uiBtn2.mouseMove(190,
		 * 1660); action4.click(startRec).build().perform();
		 */
	...
	System.out.println("Switching to the Recursive Frame..");driver.switchTo().frame(driver.findElement(By.id("recursive-social-plugin")));

	WebElement rcrscUI = driver
			.findElement(By.xpath("//div[contains(@class, 'ui recording-ui')]"));if(rcrscUI.isDisplayed())
	{
		...
		driver.findElement(By.xpath("//input[contains(@class, 'name-input form-control input-sm')]")).sendKeys("Nancy");
		...
		if (driver.findElement(By.cssSelector("body > div > a.btn.btn-success.btn-xs.sharing-button")).isDisplayed()) {
